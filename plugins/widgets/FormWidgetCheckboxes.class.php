            ...
            case 1: // utilisation des valeurs existantes
                ...
                foreach($available_values as $available_value) {
                    ...
                    $form['options'][$option_id]['weight'] = array(
                        '#type' => 'weight',
                        '#delta' => 50,
                        '#default_value' => isset($stored_values[$option_id]) ? $stored_values[$option_id]['weight'] : 50,
                    );

                    $form['options'][$option_id]['original_value'] = array(
                        '#type' => 'markup',
                        '#value' => !empty($alias) ? $alias : $option,
                    );
                }

                // on est obligé de re-trier, car la liste se base sur celle fournie par le module
                $weights = array();
                foreach ($form['options'] as $key => $option) {
                    $weights[$key] = $option['weight'];
                }
                array_multisort($weights, SORT_ASC, $form['options']);
        ...
        switch($configuration['options_mode']) {
            case 1: // utilisation des valeurs existantes
                ...
                foreach(element_children($form_state['values']['options']) as $option_id) {
                    if($form_state['values']['options'][$option_id]['enabled'] == false) {continue;}
                    ...
                    if(!empty($form_state['values']['options'][$option_id]['alias'])) {
        ...
        switch($form['configuration']['options_mode']['#default_value']) {
            case 1:
                ...
                foreach(element_children($form['options']) as $option_id) {
                    ...
                    $form['options'][$option_id]['weight']['#attributes']['class'] .= ' field-weight';
                ...
                $header = array(
                    theme('indentation', 1) . "<input id='check_uncheck_all' type='checkbox'/><label for='check_uncheck_all'>" . t("Exposed") . "</label>",
                    t("Exposed value"),
                    t("Weight"),
                    t("Value"),
                );

                $form['options']['#value'] = theme('table', $header, $rows, array('id' => 'widget_options'));
            ...
            case 3:
                if(!empty($form['options']['new_option'])) {
                    ...
                    $form['options']['#value'] .= theme('table', null, $rows);
                ...
                if(!empty($rows)) {
                    ...
                    $form['options']['#value'] = theme('table', $header, $rows, array('id' => 'widget_options')) . $form['options']['#value'];
